...
const page: FC<Params> = ({ params }) => {
    ...
    const Submit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/create-schedule/${params.name}`, {
                method: 'POST',
                headers: {
        ...
        <div className='w-full h-full flex items-center justify-center'>
            <form onSubmit={Submit} className='w-1/5 h-48  flex flex-col justify-around '>
                <div className='w-full h-auto flex'>
                    <div className='w-8 h-8 bg-green-300 rounded flex items-center justify-center'>
                        <p className='text-md font-medium'>S</p>
                    </div>
                    <h3 className='text-xl font-medium  pl-2 relative top-1 '>Create Schedule</h3>

                </div>

                <input id='name' name='name' value={scheduleData.name} onChange={inputChange} className='w-11/12 h-12 text-lg font-mono placeholder:text-gray-400 border rounded-md outline-none pl-1' type="text" placeholder='Name' />
